...
Hop.href.prototype.createRef=function(){
  var ref = JSON.parse(JSON.stringify(this.input));
  ref.href = this.toString();
  return ref;
};
...
/**
  ...
  This will render the specified template using express.js's template engine

  @param {string} template the name of the template to render
  @param {object} context the context for the template
 
  @example
    UserService.explore=function(input,onComplete,request){
      return orComplete(null,Hop.render("explore",{user:request.session.user})
    }
...
/**
  A return error or response which indicates that there was an error with one of the fields provided in the request
 
  By default this will return a json object of the form { field:, :err } and a status code of 400

  @param {string} field field associated with the error
  @param {string} errorMsg the message to be returned
 
  @example
    UserService.auth=function(input,onComplete){
      if(input.password!=input.verifyPassword)
        return onComplete(Hop.FieldError("password","The passwords do not match"));
    }
  ...
  @static
**/
Hop.FieldError=function(field,errorMsg){
  return {
    send: function(response){
      response.send(400, { field: field, msg: errorMsg });
    },
    toString:function(){ return errorMsg; }
  };
};

/**
  A return error or response which indicates that there was an authentication error
 
  By default this will return 'Invalid authentication attempt' and a status code of 400

  @param {string} msg the message to be returned
 
  @example
    UserService.auth=function(input,onComplete){
        User.auth(input,function(err,user){
          if(err)
            return onComplete(Hop.AuthError());
        })
    }
...
/**
  A return error or response which indicates that there was an internal server error
 
  By default this will return 'Internal error' and a status code of 500 and will include
  an error reference ID which is also logged via Hop.error

  @param {string} returnedError the message to be returned to the user
  @param {string} internalErrorMsg the internal error message which will be sent to the log
 
  @example
    UserService.delete=function(input,onComplete){
        User.delete(input,function(err,user){
          if(err)
            return onComplete(Hop.InternalError("Unable to delete user",err));
        })
    }
  ...
  @static
**/
Hop.InternalError=function(returnedError,internalErrorMsg){
  var errorId = Math.random().toString(36).substring(2);
  var extMsg = (returnedError||"Internal error")+" (reference Id: "+errorId+")" + (Hop.showErrors?" "+internalErrorMsg:"");
  return { 
    send: function(response){
      Hop.error("Error (reference Id: "+errorId+"):",internalErrorMsg||returnedError);  
      response.status(500);
      response.send(extMsg);
    },
    toString:function(){
      Hop.error("Error (reference Id: "+errorId+"):",internalErrorMsg||returnedError);  
      return (extMsg);
...
/**
  A return error or response which indicates that there was an DAO error
  ...
  @example
    UserService.delete=function(input,onComplete){
        User.delete(input,function(err,user){
          if(err)
            return onComplete(Hop.DAOError("Unable to delete user",err));
        })
    }
  ...
  @static
**/
Hop.DAOError=Hop.InternalError;
...
/**
  ...
  @param {string} url the message to be returned to the user
  @param {object} options options for the redirect
  @param {bool} options.noCache Do not cache this redirect 
 
  @example
    UserService.logout=function(input,onComplete,request){
      delete request.session.user;
      return onComplete(null,Hop.redirect("/login"));
    }
...
Hop.toJSON=function(noChecksum){
    ...
    if(!Hop.version){
      ...
      Hop.version=packageJSON.version;

      Hop.requiredVersions={
        "hopjs-remote":packageJSON.devDepdendencies["hopjs-remote"],
        "hopjs-workflow":packageJSON.dependencies["hopjs-workflow"]
      }
    }

    obj.Objects=Hop.Objects;
    obj.version=Hop.version;
    obj.requiredVersions=Hop.requiredVersions;
...
Hop.Object.wrap=function(objectName){
  ...
  if(obj){
    //FIXME check to make sure these are methods we want, aka not toString
...
Hop.Object.prototype.callOnError=function(method,request,input,error,stack){
  ...
  var e = error;
  if(error && error.toString) e=error.toString();
  Hop.error(method.getMethod(),":returned an error result of '",e,"'\n",stack);
  return error;
}
...
Hop.Object.prototype.create=function(name,path){
  var self = this;
  //FIXME should this overwrite an existing method?
...
Hop.Object.prototype.read=function(name,path){
  var self = this;
  //FIXME should this overwrite the method?
  this.methods[name]=new Hop.Method("get",this,name,path);
  this.methods[name].optional("expand","Which in-line hrefs to expand");
  this.methods[name].addPostCall(Hop.postCallExpand(self,name,{injectHref:true}),"data");
  return this.methods[name];
}  
...
Hop.Object.prototype.list=function(name,path){
  var self = this;
  //FIXME should this allow an overwrite?
  this.methods[name]=new Hop.Method("get",this,name,path);
...
Hop.Object.prototype.put=function(name,path){
  //FIXME make sure this doesn't overwrite the method
  this.methods[name]=new Hop.Method("put",this,name,path);      
  return this.methods[name];
}  
...
Hop.Method = function(method,object,name,_path){
  ...
  console.log("Adding",method,_path,this.getPath(true));
  Hop.router[method.toUpperCase()].add(this.getPath(true),{ method: this }); 
...
@method getPath
**/
Hop.Method.prototype.getPath=function(noBasePath){
  return Hop.Method.getPath((noBasePath?"":Hop.basePath),this._className,this);
}
...
Hop.Method.prototype.sendCSRFToken=function(){
  ...
  this.addPostCall(function Hop_Method_postCall_sendCSRFToken(req,input,err,result,next){
    if(!err && req.csrfToken){
      req._response.set('X-CSRF-Token',req.csrfToken());  
    }
    next();
  },"last");
  return this;
...
Hop.Method.prototype.call=function(input,onComplete,request,response){
  var output = {};
  var self=this;
  
  //FIXME hasOwnProperty
  for(var paramName in self.defaults){
    output[paramName]=self.defaults[paramName];  
  }
  //FIXME hasOwnProperty
  ...
  var preTasks = self._preCall.length;
  //FIXME analyze for performance
  var runPreTaskFunctions=function Hop_call_preTask(){
    if(preTasks>0){
      self._preCall[self._preCall.length-preTasks].call(request,output,onComplete,function Hop_call_preCall(){
        preTasks--;
        runPreTaskFunctions();
      },response);
    } else {
      self._object._instance[self.name](output,function Hop_call_instance(err,result,statusCode){  
          if(typeof statusCode!="undefined"){
            response.status(statusCode);
          }  
          var postTasks = self._postCall.length;
          var runPostTaskFunctions=function Hop_call_postTask(){
            if(postTasks>0){
              try {
                self._postCall[self._postCall.length-postTasks].call(request,output,err,result,function Hop_call_postCall(e,r){
