...
Hop.href.prototype.call=function(req,onComplete){
  Hop.call(this.method.getMethod(),this.input,onComplete,req);
}
...
*/  
Hop.postCallExpand=function(req,input,err,result,next){
  if(result && !err){
    ...
    doExpands=function(){
      if(expands.length>0){
        ...
        if(result[expand] && result[expand].href && result[expand].href instanceof Hop.href){
          result[expand].href.call(req,function Hop_expand_results(err,res){
            if(res){
              result[expand]=res;
              doExpands();
            } else if(err){
...
This method will expand any href's inline if asked to do so
...
Hop.Object.prototype.create=function(name,path){
  ...
  this.methods[name].addPostCall(Hop.postCallExpand,"data");
  this.methods[name].addPostCall(function(req,input,err,result,next){
    if(result){
      if(!result.href)
        Hop.warn(this.methods[name].getMethod()+" should return an href to a read method for this resource");
    } 
    next();
  },"data");
