...
/**
 * This will set the host name used to reference other objects in the returned URLs
 * by default HopJS will set this to the 'Host' header found in the request, but
 * this may not always be the right behavior if you're using Hop behind a 
 * HA setup
 *
 * @param {string} host the hostname to use, etc 'www.foo.com'
 *
 */
Hop.setHost=function(host){
  Hop._host=host;
}
...
Hop.Object.prototype.list=function(name,path){
  ...
  this.methods[name].addPostCall(function Hop_Method_postCall_list(req,input,err,result,next){
    if(result){
      ...
      input.offset=parseInt(input.offset);
      ...
      if(!result.next && itemCount>=thisInput.limit){
        ...
        for(var i in nextInput){
          result.next[i]=nextInput[i];
        }
      }
      if(!result.prev && (prevInput.offset-(prevInput.limit))>=0){
        result.prev={};
        prevInput.offset-=(prevInput.limit);
        result.prev.href = new Hop.href(self.methods[name].getMethod(),prevInput);
        for(var i in prevInput){
          result.prev[i]=prevInput[i];
        }
      }
    }
    next();
