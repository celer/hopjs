...
Hop.Method.prototype.cacheInvalidate=function(cacheId){
	...
	self.addPreCall(function(req,input,onComplete,next){
		cacheId = Hop.Cache.resolvePath(cacheId,req,input);	
...
Hop.Method.prototype.cacheId=function(cacheId,duration,force){
	...
	self.addPreCall(function(req,input,onComplete,next){
		cacheId = Hop.Cache.resolvePath(cacheId,req,input);	
		Hop.Cache.fetch(cacheId,duration,req,input,onComplete,next,force);
	});
	self.addPostCall(function(req,input,err,result,onComplete){
		cacheId = Hop.Cache.resolvePath(cacheId,req,input);	
		Hop.Cache.store(cacheId,duration,req,input,err,result,onComplete,force);
	});
	return this;
}
	...
	self.addPreCall(function(req,input,onComplete,next){
		...
		if(res && res.what){
			if(res.what=="store"){
				...
				Hop.Cache.fetch(cacheId,duration,req,input,onComplete,next,res.force);
