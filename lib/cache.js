...
**/
RAPI.Method.prototype.cacheInvalidate=function(cacheId){
	var self=this;
	self.addPreCall(function(req,input,onComplete,next){
		cacheId = RAPI.Cache.resolvePath(cacheId,req,input);	
		RAPI.Cache.invalidate(cacheId);
		next();
	});
	return this;
}
...
/**
	...
	@method cacheId
**/
RAPI.Method.prototype.cacheId=function(cacheId,duration,force){
	var self=this;
	duration=duration||(5*60);
	self.addPreCall(function(req,input,onComplete,next){
		cacheId = RAPI.Cache.resolvePath(cacheId,req,input);	
		RAPI.Cache.fetch(cacheId,duration,req,input,onComplete,next,force);
	});
	self.addPostCall(function(req,input,err,result,onComplete){
		cacheId = RAPI.Cache.resolvePath(cacheId,req,input);	
		RAPI.Cache.store(cacheId,duration,req,input,err,result,onComplete,force);
	});
	return this;
}
...
@example
		RAPI.defineClass("Comment",new Comment(),function(api){
			...
			api.get("load","/comment/:commentId").cache(function(cache,req,input){
				if(RAPI.hasUser(req)){
					var userId = RAPI.User.id(req);
	...
	@method cache
**/
RAPI.Method.prototype.cache=function(cacheFunc){
	var cache = new RAPI.Cache();
	
	var self=this;
	self.addPreCall(function(req,input,onComplete,next){
		var res = cacheFunc(cache,req,input);
		if(res && res.what){
			if(res.what=="store"){
				var cacheId = RAPI.Cache.resolvePath(res.id,req,input);	
				RAPI.Cache.fetch(cacheId,duration,req,input,onComplete,next,res.force);
	...
	self.addPostCall(function(req,input,err,result,onComplete){
		...
		if(res && res.what){
			if(res.what=="store"){
				var cacheId = RAPI.Cache.resolvePath(res.id,req,input);	
				RAPI.Cache.store(cacheId,res.duration,req,input,err,result,onComplete,res.force);
			};
		}
		return onComplete();
	});
...
RAPI.Cache.resolvePath=function(path,req,input){
	var request=req;
	RAPI.log(input);
...
RAPI.Cache.store=function(cacheId,duration,req,input,err,result,next){
	RAPI.warn("RAPI Caching is disabled");
	next();
}
...
RAPI.Cache.prototype.id=function(cacheId,duration,force){
	return { id: cacheId, duration: duration, what: "store", force: force };
}

RAPI.Cache.prototype.invalidate=function(cacheId){
