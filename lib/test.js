...
Hop.TestTask.arrayDoesntContains={ pass: "The array does not contains the specified value", fail:"The array contains the specified value, or an error occured" };
Hop.TestTask.prototype.arrayDoesntContains=function(property,tv){
  var value = { property: property, tv: tv };
  var testFunc=function(input,err,output,value){ if(output){ var p = Hop.TestUtils.getProperty(output,value.property); if(p instanceof Array) { var v = Hop.TestUtils.resolve(value.tv,true); return !Hop.TestUtils.arrayContains(p,v); }} return false; }
  this.test.push({ type:"arrayDoesntContains", value: value, testFunc: testFunc.toString() });
  return this;
}
