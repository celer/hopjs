...
Hop.Cache.clear=function(onComplete){
  ...
  Hop.cacheOptions.redisClient.flushdb(onComplete);
}
...
Hop.Cache.store=function(cacheId,duration,req,input,err,result,next,force){
  ...
  Hop.cacheOptions.redisClient.setex(cacheId,duration,JSON.stringify(entry));
  next();
}

Hop.Cache.fetch=function(cacheId,duration,req,input,onComplete,next,force){
  Hop.cacheOptions.redisClient.get(cacheId,function(err,data){
    if(!err && data){
      try {
...
Hop.Cache.invalidate=function(cacheId){
  ...
  Hop.cacheOptions.redisClient.del(cacheId);
}
