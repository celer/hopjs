...
Hop.expressAPI=function(basePath){
  ...
  Hop.workflowURL=util.webpath.join(basePath,"hopjs-wf.min.js");
  ...
  Hop.router.GET.add("/hopjs-wf.min.js",{call: send(function(){  return fs.readFileSync(Workflow.webJS).toString(); },"application/javascript")  });
  ...
  Hop.router.GET.add("/apitest.js",{ call: template("static/test/apitest.comb" ,"application/javascript") });
  ...
  return function(req,res,next){
    if(req.path.indexOf(basePath)==0){
      ...
      if(route) {
        ...
        console.log(path.join(__dirname,"static","_hopjs",url));
        fs.exists(path.join(__dirname,"..","static","_hopjs",url),function(exist){
          if(exist)
            res.sendfile(path.join(__dirname,"..","static","_hopjs",url));
          else next();
        });
      }
    } else {
