            ...
            if(err){
              ...
              if(result && result.send && typeof result.send=="function"){
                ...
                if(result===null || result===undefined){
                  ...
                  if(self.input && self.input.model){
                    res.setHeader("X-Input-Schema",util.webpath.join(Hop.basePath,"/schema/"+self.input.model));
                  }
                  if(self.output && self.output.model){
                    res.setHeader('Content-Type','application/json; profile='+util.webpath.join(Hop.basePath,"/schema/"+self.output.model));
    ...
    for(var methodName in obj.methods){
      ...
      (function Hop_apiHook(obj,method){
              app[method.method](method.getPath(),function(req,res){
                    return method.expressHandler(req,res);
                  });
      })(obj,method);
    }
  }
